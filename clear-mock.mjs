...
const emptyStateTemplate = (title) => `"use client";
    ...
    <>
      ...
      <div className="flex-1 flex items-center justify-center min-h-[500px]">
        <div className="bg-card border border-border rounded-3xl p-12 text-center max-w-md w-full shadow-sm">
          ...
          <h3 className="font-display text-2xl mb-2">No ${title.toLowerCase()} found</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-background border border-border rounded-3xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-display text-xl">New ${title} Entry</h3>
              <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-muted rounded-full transition">
                <X className="h-5 w-5" />
              </button>
            </div>
            <p className="text-muted-foreground text-sm mb-6">
              This module is currently in development. You will be able to define custom data schemas and workflows for ${title} here soon!
            </p>
...
const pagesToWipe = [
  { p: 'reservations', t: 'Reservations' },
  { p: 'guests', t: 'Guests' },
  { p: 'housekeeping', t: 'Housekeeping Tasks' },
  { p: 'finance', t: 'Financial Records' },
  { p: 'revenue', t: 'Revenue Intel' },
  { p: 'inventory', t: 'Inventory' },
  { p: 'events', t: 'Events' },
  { p: 'maintenance', t: 'Maintenance Requests' },
  { p: 'properties', t: 'Properties' },
  { p: 'inbox', t: 'Messages' },
  { p: 'loyalty', t: 'Loyalty Members' },
  { p: 'procurement', t: 'Procurement Orders' },
  { p: 'attendance', t: 'Attendance Records' },
  { p: 'payroll', t: 'Payroll Runs' },
];

pagesToWipe.forEach(({ p, t }) => {
  const filePath = path.join('./src/app/dashboard', p, 'page.tsx');
  if (fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, emptyStateTemplate(t));
  }
});
