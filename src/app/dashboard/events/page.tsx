...
import { Plus, Trash2, PartyPopper, X, Edit2 } from "lucide-react";
...
export default function EventsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    let q = query(collection(db, "events"), where("ownerId", "==", user.uid));
    if (activeProperty) {
      q = query(collection(db, "events"), where("ownerId", "==", user.uid), where("propertyId", "==", activeProperty));
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-background border border-border rounded-3xl p-6 w-full max-w-2xl shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-display text-xl">{formData.id ? 'Edit' : 'New'} Event Management Entry</h3>
            ...
            <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-muted-foreground ml-1">Expected Attendees</label>
                <input 
                  required={true} 
                  type="number" 
                  className="bg-muted border border-border rounded-xl px-4 py-3 outline-none" 
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                ...
                <th className="p-4 font-semibold text-muted-foreground">Expected Attendees</th>
              ...
              {dataList.length === 0 ? (
                ...
                dataList.map(item => (
                  <tr key={item.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-4">{item.attendees}</td>
