...
import { Plus, Trash2, Gift, X, Edit2 } from "lucide-react";
...
export default function LoyaltyPage() {
  ...
  useEffect(() => {
    if (!user) return;
    let q = query(collection(db, "loyalty"), where("ownerId", "==", user.uid));
    if (activeProperty) {
      q = query(collection(db, "loyalty"), where("ownerId", "==", user.uid), where("propertyId", "==", activeProperty));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, "loyalty", docId), {
        ...formData,
        id: docId,
        ownerId: user.uid,
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this entry?")) {
      await deleteDoc(doc(db, "loyalty", id));
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-background border border-border rounded-3xl p-6 w-full max-w-2xl shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-display text-xl">{formData.id ? 'Edit' : 'New'} Loyalty Programs Entry</h3>
            ...
            <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-muted-foreground ml-1">Guest Name</label>
                <input 
                  required={true} 
                  type="text" 
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-muted-foreground ml-1">Points Balance</label>
                <input 
                  required={true} 
                  type="number" 
                  className="bg-muted border border-border rounded-xl px-4 py-3 outline-none" 
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-muted-foreground ml-1">Membership Tier</label>
                <input 
                  required={true} 
                  type="text" 
                  className="bg-muted border border-border rounded-xl px-4 py-3 outline-none" 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                <th className="p-4 font-semibold text-muted-foreground">Guest Name</th>
                <th className="p-4 font-semibold text-muted-foreground">Points Balance</th>
                <th className="p-4 font-semibold text-muted-foreground">Membership Tier</th>
